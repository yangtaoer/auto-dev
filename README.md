...
项目策略不在云端编辑。通过 Codex 对话新增或修改 `local-runner/project-presets/*.json` 后，本机执行器最迟在下一次 20 秒心跳时自动同步；所有登录账号都能在“自助项目”页面查看。项目经理发起需求时只输入 TFS 编号，本机执行器读取工作项 Area Path，自动匹配最具体的本机项目预设和该项目的交付方式。
