...
运行看板和任务详情使用原创的“任务动力核心”把实时状态转成动画反馈：项目识别时扫描、DevCore 研发时加工、构建和发版时脉冲、等待时低频呼吸、交付时庆祝。实现只使用平台自有 HTML/CSS，不依赖第三方角色素材，并在系统开启“减少动态效果”时自动静止。设计与状态映射见 [docs/task-kinetic-design.md](docs/task-kinetic-design.md)。
