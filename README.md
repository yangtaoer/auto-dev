...
登录页右上角使用经平台所有者确认拥有使用权的 [blessonism/grok-icon-study](https://github.com/blessonism/grok-icon-study) 原始动态角色引擎，固定来源提交为 `647e9bd7c60290c42a738fad586589b3f36a4680`。平台保留该提交的原始几何、状态机、眼睛、粒子和角色源码，仅做跨平台换行归一化，并使用参考项目自身的 `onboarding / blob / black / curious` 参数初始化；来源与规范化内容哈希记录在 `app/static/grok-character/SOURCE.json`。控制台侧栏复用同一引擎，以白色主体和深色眼睛呈现，并在紧凑导航下自动暂停。登录页“交付离场”下方使用连续缓动、节点呼吸和渐隐回程的四站式项目流水线轨道，支持减少动态效果设置。
