...
登录页与控制台侧栏统一使用 AutoDev 橙色 3D 球体角色：本地 `WebGL` 着色器负责高反差球面明暗、双层接触阴影、半透明边缘、球面光纹和环境投影，经所有者确认拥有使用权的 [blessonism/grok-icon-study](https://github.com/blessonism/grok-icon-study) 原始动态引擎（固定提交 `647e9bd7c60290c42a738fad586589b3f36a4680`）则作为实时动作与表情驱动，完整保留 25 组眼型插值、弹簧位移与压缩、眨眼、视线追踪、自动情绪轮换、转身、跳跃和粒子效果。角色采用“背部环带/粒子—3D 球体—前部环带/球面眼睛”三层深度合成，眼睛按球面轮廓裁剪并在边缘渐隐，转身时不会与球体或彩色环带穿模。侧栏角色会根据执行器与任务状态在好奇、倾听、思考、工作、异常和休眠动作之间切换；不支持 WebGL 时自动显示原版平面角色，并遵循系统的“减少动态效果”设置。来源与规范化内容哈希记录在 `app/static/grok-character/SOURCE.json`。登录页“交付离场”下方使用连续缓动、节点呼吸和渐隐回程的四站式项目流水线轨道，同样支持减少动态效果设置。
