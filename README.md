...
平台会把本机研发过程中反复验证过的经验固化为项目策略和自动质量门禁，而不是只写入提示词：

- 开始研发前查询同项目、同 TFS 编号的历史任务、PR、提交和验收证据，并以最新目标分支复核；需求已经完整实现时生成验证结论，不重复创建提交或 PR。
- 每条验收标准形成独立账本，关联仓库、变更文件、测试和证据；未映射文件、未完成验收项或缺少证据会阻止提交，支持后续按验收项精确回滚。
- 统计、积分、状态、排序、导出和保存/查询链等语义改动要求记录数据不变量；SQL 会检查版本冲突、Schema 占位符、DM7 LOB 限制、变更日志与实际数据库连接核验。
- 前端变更要求真实路由、项目规定视口和截图验收，同时核对资源哈希、部署目录与缓存策略；新增视图必须同时登记菜单链接和权限绑定。
- 问题分析必须标注代码、数据库或现场环境，区分代码、数据与环境问题，并明确本次结论与历史结论的差异依据。
- 交付前按项目白名单复核产物类型。网络发令 APP 只交付本次变更端的前端 ZIP、后端 JAR 和确有变更的 SQL 脚本，不生成或展示独立 XML 与额外交付清单；其他项目按策略生成包含构建提交和 SHA256 的交付校验清单。
