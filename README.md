...
只有重要代码/业务逻辑冲突、重大安全隐患或受保护代码边界需要人工确认。普通缺项由原 DevCore 会话自动补齐，最多追加两轮；真实功能缺失、构建/测试失败仍必须如实处理，不会伪造通过。真实截图、前端资源清单/目录/缓存证据字段缺项不进入管理员审批。`delivery-validation-manifest.json` 仅供内部审计，不列入页面、邮件或新写入 TFS 的交付清单。
