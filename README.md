# AutoDev · 自主研发交付

AutoDev 是“云端控制台 + 本机 DevCore 执行器”的自主研发与问题分析平台。当前推荐的正式拓扑为：

- `auto.yangtaoer.com.cn` 解析到 `162.14.113.130`，由现有 Caddy 2 提供 HTTPS。
- Caddy 回源到 `121.4.77.6:28765`，该服务器运行一键部署的控制台容器。
- `121.4.77.6` 的 28765 只允许 `162.14.113.130/32` 访问。

- 项目经理通过 `https://auto.yangtaoer.com.cn` 登录、输入 TFS 编号并查看过程与交付物；提交成功后会立即打开任务侧栏。
- 云端保存用户、只读项目目录、任务状态、审计事件和上传后的交付产物，并发送邮件。
- 你的 Windows 电脑只主动通过 HTTPS 轮询云端，不需要公网 IP、端口映射或开放防火墙入站。
- Codex 登录态、代码仓库、TFS PAT、构建环境和审核账号全部留在本机，不上传云服务器。

三种交付方式均已接入：本机打包交付、四川专用账号审核后交付、产品部人工审核后交付。SQL、配置文件、安装包、PR 链接和合并截图/凭证均可归集到云端供项目经理下载。

平台提供两类任务：

- **自主研发**：必须产生符合需求的代码改动，并继续完成提交、审核、构建/发版和交付。
- **问题分析**：结合 TFS 内容、项目代码、配置、日志和本机 DM7 开发库进行只读核验；零代码改动即为正常结果。平台会输出根本原因、可信度、证据链、影响范围、数据库核验记录和建议动作，生成可下载的 Markdown 分析报告，把报告获取路径回填 TFS，并在分析交付完成后将需求状态更新为“已解决”。问题分析不会提交代码、创建 PR、构建、发版或创建 License 申请；信息不足时进入“待补充分析信息”，由提交人补充后继续原分析会话。

登录页与控制台侧栏统一使用 AutoDev 橙色 3D 球体角色：本地 `WebGL` 着色器负责高反差球面明暗、双层接触阴影、半透明边缘、球面光纹和环境投影，经所有者确认拥有使用权的 [blessonism/grok-icon-study](https://github.com/blessonism/grok-icon-study) 原始动态引擎（固定提交 `647e9bd7c60290c42a738fad586589b3f36a4680`）则作为实时动作与表情驱动，完整保留 25 组眼型插值、弹簧位移与压缩、眨眼、视线追踪、自动情绪轮换、转身、跳跃和粒子效果。角色采用“背部环带/粒子—3D 球体—前部环带/球面眼睛”三层深度合成，眼睛按球面轮廓裁剪并在边缘渐隐，转身时不会与球体或彩色环带穿模。控制台增加业务语义动画导演层：角色会按需求读取、方案校验、研发、构建、交付、阻塞、成功和执行器休眠切换神态，跟随当前任务卡片转动视线；侧栏固定宽度信息窗展示任务当前输出，多任务时按 `1/N` 轮换且最多显示两行。新任务和步骤推进会发出能量信号，阻塞时主动提示需要判断，交付成功时用真实产物类型形成短暂的离场标签。球外旋转弧线默认隐藏，仅在需求准入、研发、提交、构建或交付实际执行期间出现；登录、空闲、排队和等待人工处理时不显示。点击角色还能触发轻量彩蛋。所有效果在后台标签页停止渲染，不支持 WebGL 时自动显示原版平面角色，并遵循系统的“减少动态效果”设置。来源与规范化内容哈希记录在 `app/static/grok-character/SOURCE.json`。登录页“交付离场”下方使用连续缓动、节点呼吸和渐隐回程的四站式项目流水线轨道，登录成功与失败也会触发角色反馈，同样支持减少动态效果设置。

本机 Runner 可将生成的交付物直接上传阿里云 OSS。云端控制台和交付邮件均使用 OSS 预签名下载链接；默认链接有效期与产物保留期均为 3 天。Runner 启动时执行一次清理，之后每 72 小时删除 `autodev/` 前缀下超过 3 天的 OSS 对象及本机临时交付目录。交付邮件使用 AutoDev 品牌模板，时间统一显示为北京时间（UTC+8），发件人显示名可通过 `SMTP_FROM_NAME` 配置；所有任务邮件会自动抄送 `AUTODEV_TASK_ADMIN_EMAIL`，默认值为 `yangtao2@tellhow.com`。

## 目录

- `deploy/backend/`：`121.4.77.6` 后端服务器一键部署。
- `deploy/edge-caddy/`：`162.14.113.130` 现有 Caddy 的站点配置。
- `deploy/cloud/`：控制台与 Caddy 部署在同一台服务器时使用的备用方案。
- `local-runner/`：Windows 本机执行器安装、启动和登录自启脚本。
- `local-runner/project-presets/`：项目策略的唯一配置源；本机执行器自动同步云端只读目录。
- `local-runner/project-scripts/`：项目专用构建/打包脚本，避免每次任务临时推断命令。
- `app/`：云端控制台与本机编排器共用源码。
- `docs/deployment.md`：从 DNS 到正式运行的完整部署清单。
- `docs/architecture.md`：系统边界、三种流程与恢复行为。

## 最短上线步骤

在 `121.4.77.6` 解压部署包后：

```sh
cd deploy/backend
chmod +x *.sh
./deploy.sh
```

部署脚本已预置 163 SMTPS/465 邮件配置，会自动生成 `.env.backend`、管理员初始密码、会话密钥和本机执行器令牌，然后把应用发布到宿主机高位端口 28765。随后把 `deploy/edge-caddy/auto.yangtaoer.com.cn.caddy` 合并到 `162.14.113.130` 的现有 Caddyfile 并 reload。完整命令见 [docs/two-server-deployment.md](docs/two-server-deployment.md)。

本机 Windows 上：

```powershell
.\local-runner\install.ps1
# 编辑 local-runner\.env.runner，并安全写入 runner-token.txt 与 TFS PAT
.\local-runner\start.ps1
```

需要随 Windows 开机自动运行：

```powershell
.\local-runner\install-startup-task.ps1
```

首次执行会弹出 Windows UAC 管理员确认。该命令会注册开机、用户登录和每 5 分钟恢复三类触发器。开机初期网络或云端暂不可用时，执行器会保持运行并自动重连，不再因首次心跳失败退出。

本机运行管理：

```powershell
.\local-runner\client.ps1          # 打开图形化执行器控制台
.\local-runner\status.ps1          # 计划任务、进程、云端连通和最近日志
.\local-runner\logs.ps1 -Follow    # 持续查看执行日志
.\local-runner\stop.ps1            # 停止
.\local-runner\restart.ps1         # 重启
```

`install.ps1` 会在桌面创建“AutoDev 执行器控制台”快捷方式。该 C/S 客户端可查看当前与历史任务、任务步骤、执行器日志和打开期间的 Codex 详细输出，并可启动、停止、重启本机执行器。本机监控接口仅监听 `127.0.0.1:28766`；Codex 详细输出只在查看窗口打开期间进入内存，关闭窗口后立即停止采集且不持久化。

本机 Codex 默认复用当前电脑已有的 Codex/ChatGPT 登录态，一般不需要配置 API key。

OSS 参数配置在本机 `local-runner/.env.runner`，包括 AccessKey、Region、Endpoint、Bucket、对象前缀、链接有效期、保留天数和清理周期。OSS 凭据不会进入云端容器或一键部署 ZIP。

单个交付文件默认允许小于 **1 GB（1024 MB，即 1,073,741,824 字节）**；达到或超过该大小会被拒绝。本机 `local-runner/.env.runner` 和云端 `deploy/backend/.env.backend` 的 `AUTODEV_MAX_ARTIFACT_MB` 应均设为 `1024`。旧安装的环境文件会被升级程序保留，如仍配置为 `200`，需调整该项并重启对应服务。Runner 将文件流直接上传 OSS，云端与邮件只保存下载链接，不经过 Caddy 传输安装包。

## 项目与账号配置

项目策略不在云端编辑。通过 DevCore 对话新增或修改 `local-runner/project-presets/*.json` 后，本机执行器最迟在下一次 20 秒心跳时自动同步；管理员可在“自主项目”页面查看同步后的只读目录、项目别名及每个项目实际配置的 TFS Git 仓库路径，项目经理不显示此管理菜单。项目经理发起需求时只输入 TFS 编号，本机执行器会综合 Area Path、标题、需求说明和验收标准，自动归类到一个或多个项目及其交付方式。

多项目需求建议在标题中连续使用标准项目名，例如 `【网络发令APP】【四川省调网络发令】新增联合功能`。系统会为每个项目生成独立、可并行执行的研发子任务，将正文拆为项目专属条目和跨项目共同条目；各子任务分别完成代码提交、PR 审核、构建或发版，全部结束后只统一更新一次 TFS 状态与交付产物，并发送一封汇总交付邮件。单次联合研发最多支持 5 个项目。

项目预设关键项：

- `本机执行器 ID`：默认 `yangtao-pc`，必须与 `.env.runner` 一致。
- `本机代码仓库绝对路径`：例如 `C:\workspace\project`，由本机执行器使用。
- `TFS Collection / 项目 / Area Path`：用于需求准入和 PR 操作。
- `基础分支`：默认 `dev`。PR 审核类项目只推 feature 分支；本地打包项目会先同步最新基础分支，将本次提交推送到基础分支并校验远端提交一致，再基于该最新代码构建。
- `构建/校验命令`、安装包匹配、SQL/配置匹配和受保护路径。
- 三种交付方式之一；项目经理发起时无需选择。

平台会把本机研发过程中反复验证过的经验固化为项目策略和自动质量门禁，而不是只写入提示词：

- 开始研发前查询同项目、同 TFS 编号的历史任务、PR、提交和验收证据，并以最新目标分支复核；需求已经完整实现时生成验证结论，不重复创建提交或 PR。
- 从需求描述拆解并冻结验收账本，不使用 TFS 的验收条件字段；有分点时保留原分点及解释，无分点时可整体验收。每项关联仓库、变更文件、测试和证据；未映射文件、真实功能缺项或缺少必要证据必须如实处理。
- 任务详情使用全局弹窗，按“交付概览 → 需求拆解 → 研发过程 → 合并发版 → 验收反馈”分页；产出物在首页，内容仅在当前 Tab 内滚动，后台刷新保留选中页及输入草稿。
- 提出人只需选择“验证通过 / 验证不通过”；失败项和说明均可不填，交付版本自动关联。整体通过才将全部项记为人工通过；整体不通过但未指定项时保留未验证状态，不推断其余项已通过。反馈仍是追加式、幂等且有并发版本校验，单独点击继续修复才启动返修。未指定失败范围时先排查需求差异，再精确修复。管理员仍可在项目经验页查看完整积累。
- 历史上尚无人验收的旧 TFS 验收清单，会在已交付任务读取时按保存的需求描述一次性更新；已有人工反馈或返修关联的旧清单保持不变，以免重编号破坏追溯。界面回归可运行 `python scripts/check-task-dialog.py`（需 Playwright 与本机 Edge，使用临时数据库，不操作真实任务）。
- 统计、积分、状态、排序、导出和保存/查询链等语义改动要求记录数据不变量；SQL 会检查版本冲突、Schema 占位符、DM7 LOB 限制、变更日志与实际数据库连接核验。
- 前端变更以 production 构建、真实路由、自动断言、资源哈希、部署目录与缓存策略作为交付依据；真实页面截图对所有项目都不是阻塞门禁。新增视图仍必须登记菜单链接和权限绑定。
- 网络发令 APP 与其他项目统一使用 production 构建、真实路由和自动断言作为可复核证据，不能因需求提到截图或浏览器后端不可用而暂停研发。
- 网络发令 APP 执行精确开发：先核对现有功能、共享组件、真实路由、接口调用链和区域分支，再按验收项做最小改动；禁止顺手重构、批量格式化或改变无关地市与公共默认行为。修改共享接口、缓存、登录态、权限或消息协议时必须检查全部调用方，并回归登录、首页、待办/角标/列表数据范围、详情提交、区域隔离、接口白名单和构建打包。
- 问题分析必须标注代码、数据库或现场环境，区分代码、数据与环境问题，并明确本次结论与历史结论的差异依据。
- 交付前按项目白名单复核产物类型。网络发令 APP 只交付本次变更端的前端 ZIP、后端 JAR 和确有变更的 SQL 脚本，不生成或展示独立 XML 与额外交付清单；其他项目按策略生成包含构建提交和 SHA256 的交付校验清单。

当前本机目录已启用：巴中自巡航-自研（本地打包）、成都网络发令（四川自动审核）和南充网络发令（产品审核）。成都与南充均按多仓库项目处理，所有仓库统一从 `dev` 创建需求分支。

管理员可在云端创建、编辑、启用或禁用账号。每个账号支持最多 10 个通知邮箱；发起需求时从当前账号维护的邮箱中多选本次收件人，默认全部选中。

管理员任务总览展示当日任务、任务总量、成功、失败、运行中和等待合并等全局指标，同时显示本机 Codex 套餐类型、周期剩余额度、重置时间和额外余额。正在运行卡片不展示推测性百分比，只展示最近一条精简执行动态；任务处于 Codex 研发阶段时，详情侧栏提供“查看研发过程”实时会话入口。

任务因高风险或技术阻塞进入“等待人工确认”后，详情首屏和通知邮件会同时说明“为什么阻塞”与“需要你判断”，并把常见技术检查项转成可读的判断问题。管理员可在详情中填写判断依据或补充提示后继续执行。系统不会创建重复任务，而是采用最新项目门禁、复用原隔离工作区和原 DevCore 会话继续尝试解决，并保留完整审计记录。

四川自动审核方式必须配置构建/校验命令，且应使用经组织授权的专用审核账号。Codex 报告风险、构建失败或触及保护路径时，不会自动批准 PR。

## 断线与恢复

平台研发与问题分析默认使用 **GPT-6 Astra（`gpt-6-astra`）**，不跟随桌面当前选中的模型，也不会自动降级。`CODEX_MODEL` 可显式配置；为空时仍使用 Astra。

Python SDK 和实际 Codex CLI 分开管理：SDK 使用 `requirements-runner.txt`，独立 CLI 固定在 `local-runner/codex-runtime/package.json`（当前已验证 0.153.4）。升级 SDK 并不等于升级实际模型运行器。安装/更新执行器时，在没有活动任务的情况下运行 `local-runner/install.ps1`（需要 Node.js 18+），同步两类依赖后重启执行器；已有 `.venv` 也会同步。CLI 的 `node_modules` 不提交或打进云端部署包。

默认优先使用平台独立 CLI，其次查找 PATH 中的原生 Codex；可设置 `CODEX_BIN` 指向新版原生 `codex.exe`。不使用 SDK 捆绑的旧 CLI，不通过更换模型绕过版本错误。任务事件及执行器心跳记录实际模型、CLI/SDK 版本；额度、认证、连接、版本错误会脱敏后展示在任务失败原因和邮件中。

更新后可用执行器同一 Python 环境进行真实模型连接检查（不会修改业务仓库）：

```powershell
$env:AUTODEV_ENV_FILE = "local-runner/.env.runner"
.\.venv\Scripts\python.exe scripts/codex_smoke.py
```

- 本机电脑关机或执行器退出时，新任务保留在云端队列，项目经理仍可正常访问控制台。
- 本机重新上线后继续领取排队任务，并继续轮询等待合并的 PR。
- 已上传云端的安装包、SQL、配置和截图不依赖本机在线。
- 为避免重复提交代码，已进入研发/提交阶段但因本机异常中断的任务不会盲目自动重跑，应先检查分支/PR 后人工处理。

## 验证

```powershell
.\.venv\Scripts\python.exe -m unittest discover -s tests -v
```

测试使用临时数据库和演示项目，覆盖三种交付链路、云端执行器协议、临时 Codex 实时会话、管理员看板和项目经理权限，不访问真实 TFS、不调用 Codex、不发送真实邮件。

完整部署步骤见 [docs/deployment.md](docs/deployment.md)。

## 从 GitHub 一键更新

每次代码改动完成后，本机发布流程会运行测试、推送 `main`，并通过 `scripts/publish-github.ps1` 覆盖发布固定名称的 `autodev-hybrid-latest.zip`。GitHub Actions 也保留了手工触发入口。服务器首次安装包含更新脚本的版本后，以后只需：

```sh
cd /opt/autodev/deploy/backend
./update-from-github.sh
```

脚本会下载并校验最新 ZIP、备份 SQLite 数据库与本地产物、覆盖程序文件并滚动更新容器。`.env.backend`、`secrets/` 和 `data/` 不包含在 ZIP 中，因此不会被覆盖。

需要让服务器自动跟随 GitHub `latest` 时执行一次：

```sh
cd /opt/autodev/deploy/backend
./install-auto-update.sh
```

该脚本安装 systemd timer，每 5 分钟检查一次发布包校验值。版本未变化时不做任何操作；有新版本时自动校验、备份并升级。查看状态和日志：

```sh
systemctl status autodev-github-update.timer
journalctl -u autodev-github-update.service -n 100 --no-pager
```

### 自主执行与全局模型配置

任务总览将套餐容量和任务计数合并为紧凑信息栏，最近任务最多显示 12 条，超出高度在表格内部滚动。

所有受平台支持的项目共用提交过滤：`test/`、`tests/` 中的 Python 文件、`test*.py`、`*_test.py`、`conftest.py` 及任意层级 `docs/` 下的 JSON 不进入提交；业务代码、SQL、XML、Markdown 不受此规则影响。规则同时写入研发提示和暂存区检查，文件重命名按原路径/新路径整体排除。已有仓库文件不会被删除，排除的改动留在本地；本地打包同步目标分支前，必要时保存到本地 Git stash，并在执行记录中记录 stash 标识，避免临时验证材料阻碍 rebase。不会推送这些 stash，也不会自动改写已存在的远端提交。

管理员进入「研发设置」选择全局模型及默认思考深度。选项来自本机 Codex CLI 的 `model/list`，不是静态猜测；初始使用 GPT-6 Astra / high。设置保存在云端数据库并记录审计，新开工任务会冻结配置，运行中和原会话续作不受后来修改影响。执行器暂时离线不会清除上次已确认的模型列表。

只有重要代码/业务逻辑冲突、重大安全隐患或受保护代码边界需要人工确认。普通缺项由原 DevCore 会话自动补齐，最多追加两轮；真实功能缺失、构建/测试失败仍必须如实处理，不会伪造通过。真实截图、前端资源清单/目录/缓存证据字段缺项不进入管理员审批。“配合现场测试无误”等纯交付后现场验收条目保持待验证，不反过来阻塞研发交付；条目含实际功能开发时不适用此例外。`delivery-validation-manifest.json` 仅供内部审计，不列入页面、邮件或新写入 TFS 的交付清单。

选择自动发版的审核交付需求，合并后会原子登记项目发版批次。同项目仍有研发、排队、构建或待合并需求时，先完成者显示「等待同项目合并发版」；最后就绪者统一发版，兼容分支/仓库范围的待发版需求共享同一产物。其他项目、只读分析和已暂停/失败/取消需求不会无限拖住发版。发版启动后才加入的需求不冒充已被该批次覆盖；批次失败不会盲目重复创建流水线。

TFS 只读连接异常会更换连接并退避重试。提交前的准入读取若仍不可用，保留开始时间、工作区与原会话，稍后自动重试，不发送失败终止邮件。历史 `WinError 10053/10054` 准入失败且尚未提交 PR 的任务，可由管理员点击「继续执行并保留现场」。写入响应丢失不自动重放，以免重复创建 PR、发版或修改 TFS。

### 取消、重新开始与代码回退

管理员和任务发起人可在任务弹窗「交付概览」操作，所有命令和执行进展持久化留痕：

- **取消**：中断当前 DevCore 会话、停止执行器启动的本地构建进程树，等待旧工作退出后关闭尚未合并的 PR。已合并代码、已启动的外部发版不会自动撤销。
- **重新开始**：先完成上述停止流程，再读取最新需求与项目配置，从最新目标分支创建新任务、新工作区和新会话；不删除原记录。验收返修保留未通过范围及已通过保护项。联合任务仅重开当前项目，取消按钮则取消整个联合任务。
- **回退需求**：仅已交付的代码研发任务可用。校验原 PR 或精确提交映射，在隔离工作区生成反向提交（不 reset、不强推目标分支），保留后续其他需求的改动。所有仓库先准备并校验，再按原交付策略推送目标分支或创建审核 PR；需人工审核的项目仍等待审核通过。项目存在在途任务时等待，其间避免同项目新任务与回退并发。

回退只处理代码，不自动执行数据库回滚、打包、发版或环境部署。发生冲突、历史任务提交映射不可信、执行器中途退出或写入结果不明时，保留提交/PR/工作区线索，不盲目重复回退；多仓库不具备跨仓原子提交，部分成功会逐仓记录。回退完成后停用原任务的项目经验，原任务及验收记录仍保留用于追溯。

巴中自巡航的主交付分支为 `dev`。本地交付顺序为「业务代码提交 → 同步并推送 `origin/dev` → 校验构建 HEAD 与已推送提交一致 → 本地前后端打包」。执行器单独保存本次需求的提交列表与构建提交，避免把并发合入的其他需求当成本次回退范围；巴中脚本基于本次改动清单收集 SQL（保留目录层级），不再使用合入后为空的 `origin/dev...HEAD` 差异。未推送或已跟踪文件未提交时禁止打包。
