...
登录页与控制台侧栏统一使用 AutoDev 橙色 3D 球体角色：由本地 `WebGL` 着色器实时生成球体高光、半透明边缘、环境投影、胶囊眼睛、呼吸、眨眼和视线跟随，不依赖外网资源；侧栏角色会根据执行器与任务状态在好奇、倾听、思考、工作、异常和休眠动作之间切换，并遵循系统的“减少动态效果”设置。平台同时保留经所有者确认拥有使用权的 [blessonism/grok-icon-study](https://github.com/blessonism/grok-icon-study) 原始动态角色引擎（固定提交 `647e9bd7c60290c42a738fad586589b3f36a4680`），仅作为不支持 WebGL 时的橙色离线降级；来源与规范化内容哈希记录在 `app/static/grok-character/SOURCE.json`。登录页“交付离场”下方使用连续缓动、节点呼吸和渐隐回程的四站式项目流水线轨道，同样支持减少动态效果设置。
