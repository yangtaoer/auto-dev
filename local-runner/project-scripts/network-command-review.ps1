param(
    [string]$DefaultBaseBranch = "dev",
    [string]$StarterBaseBranch = ""
)
...
Get-ChildItem -LiteralPath $WorkspaceRoot -Directory | ForEach-Object {
    ...
    if ($_.Name -eq "dcsd-springboot-starter" -and $StarterBaseBranch) {
        $BaseBranch = $StarterBaseBranch
    }
    & git -C $Repository rev-parse --verify --quiet "origin/$BaseBranch"
    if ($LASTEXITCODE -ne 0) {
        throw "仓库 $($_.Name) 缺少基础分支 origin/$BaseBranch"
    }
    & git -C $Repository diff --quiet "origin/$BaseBranch...HEAD" --
    $DiffExitCode = $LASTEXITCODE
    if ($DiffExitCode -eq 1) {
        $ChangedRepositories += [pscustomobject]@{
            Repository = $_
            BaseBranch = $BaseBranch
        }
    }
    elseif ($DiffExitCode -ne 0) {
        throw "无法检查仓库 $($_.Name) 的 $BaseBranch 分支差异，git 退出码：$DiffExitCode"
    }
}
...
foreach ($Changed in $ChangedRepositories) {
    ...
    Write-Host "正在构建变更仓库：$($Repository.Name)（基础分支：$($Changed.BaseBranch)）"
