...
class WorkflowTests(unittest.TestCase):
    ...
    def test_acceptance_is_frozen_from_description_not_tfs_acceptance_field(self) -> None:
        from app.services import project_learning
        project_id = self.create_project("description-acceptance", "local_package")
        item = {"id": 910138, "title": "按需求拆解", "description": "<ol><li>登录保留原角色</li><li>查询当前单位待办</li></ol>",
                "acceptance_criteria": "错误的旧验收条件：必须现场截图且等待管理员确认", "revision": 7}
        with patch.object(worker, "_validate", return_value=item):
            detail = self.submit_and_process(project_id, item["id"])
        acceptance = project_learning.get_acceptance(detail["id"])
        self.assertEqual([row["criterion"] for row in acceptance["items"]], ["登录保留原角色", "查询当前单位待办"])
        self.assertTrue(all(row["source"] == "requirement_description_v2" for row in acceptance["items"]))
...
""",
    ...
    def test_new_intake_is_immediately_visible_and_home_shows_version(self) -> None:
        ...
        page = self.client.get("/")
        self.assertEqual(page.text.count("SYSTEM V1.0-Alpha.42"), 1)
        ...
        self.assertIn("mode: 'manual'", page.text)
        ...
        self.assertIn("launchOrbSignal", script)
        self.assertIn("releaseDeliveryTokens", script)
        self.assertIn("collectOrbTransitions", script)
        self.assertIn("flushOrbTransitions", script)
        self.assertIn("feeds-next", script)
        ...
        self.assertIn("mode: 'onboarding'", login_template)
        self.assertIn("ambient: true", login_template)
        self.assertIn("state: 'curious'", login_template)
        self.assertIn("sizePx: 124", login_template)
    ...
    def test_project_menu_is_hidden_from_project_manager(self) -> None:
        ...
        self.assertIn("editorial-ui.css?v=1.0-Alpha.42-login", login_page.text)
        self.assertIn("autodev-sidebar-mark.png?v=1.0-Alpha.42", login_page.text)
        ...
        self.assertEqual(pm_page.text.count("SYSTEM V1.0-Alpha.42"), 1)
    ...
    def test_login_character_matches_authorized_reference_source(self) -> None:
        ...
        self.assertIn("new global.GrokCharacter", orb_script)
        self.assertIn("this.motionDriver.moodN = 1", orb_script)
        self.assertIn("this.motionDriver.extras?.turn", orb_script)
        self.assertIn("this.motionDriver?.spinOnce", orb_script)
        self.assertIn("this.motionDriver?.bounceOnce", orb_script)
        self.assertIn("this.motionDriver?.burstOnce", orb_script)
        self.assertIn("dispatchOnce(target)", orb_script)
        self.assertIn("progressOnce(target)", orb_script)
        self.assertIn("celebrateOnce(target)", orb_script)
        self.assertIn("alertOnce(target)", orb_script)
        self.assertIn("this.motionDriver?.setGazeTarget(point)", orb_script)
        self.assertIn("等待你给出判断依据", orb_script)
        self.assertIn("float radius = 0.882", orb_script)
        self.assertIn("_initializeDepthLayers", orb_script)
        self.assertIn("this.root.insertBefore(backSvg, this.canvas)", orb_script)
        self.assertIn("orb-sphere-mask", orb_script)
        self.assertIn("orb-eye-depth", orb_script)
        self.assertNotIn("sdCapsule", orb_script)
    ...
    def test_login_uses_optimized_delivery_line_background_video(self) -> None:
        ...
        self.assertIn("background: #fbf8f0", editorial_styles)
        self.assertIn(".login-shell.is-authenticating .login-panel { opacity:1; transition:none; }", editorial_styles)
        self.assertNotIn(".login-shell.is-authenticating .login-panel { opacity:.68", editorial_styles)
        self.assertIn("grid-template-rows: auto auto", editorial_styles)
        self.assertIn("width: clamp(230px, 58cqw, 340px)", editorial_styles)
        self.assertIn("font: 900 clamp(44px, 12.5cqw, 70px)", editorial_styles)
        self.assertIn("height: clamp(82px, 20.5cqw, 122px) !important", editorial_styles)
