...
24. 区分研发实现与交付后验收：“配合现场测试无误”等纯现场/用户验收条目，在尚未部署或没有现场结论时必须保留 partial 并记录待验收，不得伪造现场测试通过。这类后续验收安排仅记入 risks，不写 blocking_risks，也不索要管理员确认。条目中包含的实际功能、接口或代码修改仍须完整实现和自检；不能把未完成开发包装为待现场验收。
""".strip()
