...
const state = { projects: [], users: [], dashboard: {active:[],recent:[],counts:{},runners:[],stats:{}}, filter:'all', selectedRequest:null, selectedIntake:null, routingGeneration:0, live:{requestId:null,watcherId:null,cursor:0,generation:0,timer:null,lastGroup:'',lastKind:'',lastBubble:null} };
const MODE = {
...
async function openRoutingDetail(intakeId,workItemId){state.selectedIntake=intakeId;state.selectedRequest=null;const generation=++state.routingGeneration;showDetailDrawer();document.querySelector('#detail-content').innerHTML=`<div class="detail-head routing-head"><p class="eyebrow">ROUTING / ${escapeHtml(intakeId.slice(0,8))}</p><h2>#${workItemId} · 正在识别研发项目</h2><div class="detail-meta"><span>任务已成功发起</span><span>等待本机执行器</span></div></div><div class="routing-signal"><i></i><div><b>正在读取 TFS 需求</b><span>系统会自动识别所属项目与交付策略，识别完成后此处直接切换为任务详情。</span></div></div>`;try{const requestId=await waitForRouting(intakeId);if(generation!==state.routingGeneration||state.selectedIntake!==intakeId)return;if(requestId){state.selectedIntake=null;toast('项目已识别，研发任务进入队列');await refresh();await openDetail(requestId)}else{document.querySelector('#detail-content').innerHTML='<div class="error-box">项目识别仍在后台进行，请稍后从交付记录中查看。</div>'}}catch(err){if(generation!==state.routingGeneration)return;document.querySelector('#detail-content').innerHTML=`<div class="detail-head"><p class="eyebrow">ROUTING FAILED</p><h2>#${workItemId} · 项目识别失败</h2></div><div class="error-box">${escapeHtml(err.message)}</div><div class="detail-actions"><button class="btn btn-secondary" id="retry-new-run">重新发起</button></div>`;document.querySelector('#retry-new-run')?.addEventListener('click',()=>{closeDetail();document.querySelector('#open-request').click()})}}
async function openDetail(id){state.selectedIntake=null;state.selectedRequest=id;showDetailDrawer();await refreshDetail(id)}
...
function appendCodexEvent(event){const live=state.live,chat=document.querySelector('#codex-chat'),kind=event.kind||'status',group=event.group||`seq-${event.seq}`;let bubble=live.lastBubble;if(!event.delta||group!==live.lastGroup||kind!==live.lastKind||!bubble){bubble=document.createElement('article');bubble.className=`chat-message ${kind}`;const labels={assistant:'CODEX',reasoning:'ANALYSIS SUMMARY',command:'TERMINAL',file:'FILE CHANGE',plan:'PLAN',status:'SYSTEM'};bubble.innerHTML=`<div class="chat-label"><span>${labels[kind]||'SYSTEM'}</span><time>${new Date(event.at).toLocaleTimeString('zh-CN',{hour12:false})}</time></div><pre></pre>`;chat.appendChild(bubble);live.lastBubble=bubble}bubble.querySelector('pre').textContent+=event.content||'';live.lastGroup=group;live.lastKind=kind;while(chat.children.length>140)chat.removeChild(chat.firstElementChild);chat.scrollTop=chat.scrollHeight}
...
function closeDetail(){state.routingGeneration+=1;state.selectedIntake=null;state.selectedRequest=null;closeCodexStream();document.querySelector('#detail-backdrop').hidden=true;document.querySelector('#detail-drawer').classList.remove('open');document.querySelector('#detail-drawer').setAttribute('aria-hidden','true')}
