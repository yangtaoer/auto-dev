...
5. 本机执行器会自动把预设同步为云端只读项目目录，所有账号均可查看；发起时只填 TFS 编号，执行器综合 Area Path、标题、需求说明与验收标准识别项目。
6. 涉及多个项目时，TFS 标题建议连续填写标准项目名，例如 `【网络发令APP】【四川省调网络发令】需求标题`。系统最多并行拆分 5 个项目，并在全部子任务完成后统一交付。
